...
def pull_reg_gov_data(
    ...
    param_dict = {
        "comments": {
            ...
            "sort_fields": "lastModifiedDate,documentId",
        },
        "documents": {"commentOnID_filter": None, "sort_fields": "lastModifiedDate"},
        "dockets": {"commentOnID_filter": None, "sort_fields": "lastModifiedDate"},
    }

    all_json_objects = []
    unique_json_objects = {}

    params.update({"page[size]": 250, "sort": param_dict[data_type]["sort_fields"]})
    # update if we need additional filtering for comments
    if param_dict[data_type]["commentOnID_filter"] is not None:
        params.update(param_dict[data_type]["commentOnID_filter"])
    ...
    while continue_fetching:
        ...
        if success:
            all_json_objects.extend(r_json["data"])
            print(
                f"Fetched {len(r_json['data'])} objects, total: {len(all_json_objects)}"
            )
            ...
            if len(r_json["data"]) < 250 or not has_next_page:
                print("No more pages or fewer than 250 objects fetched, stopping...")
                ...
                params = {
                    ...
                    "page[size]": 250,
                    "sort": param_dict[data_type]["sort_fields"],
                    "page[number]": 1,
                    "api_key": api_key,
                }
                # update if we need additional filtering for comments
    ...
    for json_object in all_json_objects:
        unique_json_objects[json_object["id"]] = json_object
    return list(unique_json_objects.values())
